...
Even if we did choose a scheme structure on $Z$, the category $bold("QCoh")(Z)$ is not a full subcategory of $bold("QCoh")(X)$
(think of Bockstein).
...
Instead, the example indicates that a closed _subset_ $Z subset X$ of a scheme should not be considered analogous to a closed submanifold of a manifold in the first place.
...
In a sense, this entire project arose from trying to apply this geometric interpretation to the structures we encounter in chromatic homotopy theory.

== Geometric backgrounds
The idea that purely category-theoretic information suffices to "do some geometry" goes back at least since Grothendieck.
The Grothendieck--Serre approach to geometry was first to consider topological spaces $X$ together with a sheaf of local rings $cal(O)_X$ on $X$.
...
Let's unpack.
We obtain a symmetric monoidal category $cal(A)_bold(X) (X)$ for each object $X in bold(X)$.
...
If $f in bold(E)$, then we also have an adjunction $(f_!,f^!)$ between $cal(A)_bold(X) (X)$ and $cal(A)_bold(X) (Y)$ in which the left adjoint $f_!$ is a map of $cal(A)_bold(X)(Y)$-modules, so we get a projection formula $f_!(F times.circle f^ast G) = f_! F times.circle G$.
...
#h(-1.2em) in which $f in bold(E)$, then we obtain identifications of the left adjoints $g^ast f_! = f_! g^ast$ and of the right adjoints $g^! f_ast = f_ast g^!$.
